import "server-only";
import type { CasesInterviewTaskRequestFactsPort, InterviewTaskRequestFacts, TaskFactsTransaction, TaskFactsAssigneeRole } from "../../shared/public.ts";
...
export class PostgresqlInterviewTaskRequestFacts implements CasesInterviewTaskRequestFactsPort {
  ...
  async readFacts(tx:TaskFactsTransaction,input:Readonly<{organizationId:string;targetId:string;invitationId:string}>):Promise<InterviewTaskRequestFacts|null> {
    ...
    const result=await tx.query<{case_id:string;interview_at:Date|string;owner_user_id:string;role:TaskFactsAssigneeRole;
      membership_id:string;binding_id:string;actor_user_id:string;business_category:string|null}>({text:`SELECT
      c.id AS case_id,f.interview_at,c.primary_user_id AS owner_user_id,b.role,m.id AS membership_id,b.id AS binding_id,
    ...
    return {sourceEventId,invitationId:input.invitationId,caseId:row.case_id,targetId:input.targetId,
      interviewAt:new Date(row.interview_at).toISOString(),ownerUserId:row.owner_user_id,assigneeRole:row.role,
