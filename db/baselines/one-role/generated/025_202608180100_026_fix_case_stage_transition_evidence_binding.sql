...
DECLARE
  ...
  bound_assessment_id uuid;
  bound_manifest_id uuid;
BEGIN
  ...
  IF OLD.stage = 'signed' AND NEW.stage = 'background_collection' THEN
    ...
    SELECT assessment.id, assessment.manifest_id, assessment.status, manifest.status
      INTO bound_assessment_id, bound_manifest_id, assessment_status, manifest_status
       ...
       OR EXISTS (
         SELECT 1
           ...
           LEFT JOIN public.cases_assessment_answers AS answer
             ON answer.assessment_id = bound_assessment_id
            AND answer.organization_id = NEW.organization_id
            AND answer.manifest_id = bound_manifest_id
            AND answer.field_id = field.field_id
          WHERE field.manifest_id = bound_manifest_id
