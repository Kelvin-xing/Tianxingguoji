...
`db/migrations/manifest.json`; the historical 33 migration files and their
hashes are immutable.

The generator reads all 33 frozen sources, applies six explicit one-role
transforms (008, 011, 012, 013, 025, and 028), copies the other 27 files byte-for-byte,
and emits 34 ordered SQL files plus a manifest. `db:baseline:check` regenerates
...
a random loopback port, and the real 34-file runner. It requires a dry-run
