...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'tianxing_test_application') THEN
    CREATE ROLE tianxing_test_application NOLOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE
      INHERIT NOREPLICATION NOBYPASSRLS;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'tianxing_test_identity') THEN
    CREATE ROLE tianxing_test_identity NOLOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE
      INHERIT NOREPLICATION NOBYPASSRLS;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'tianxing_test_provisioner') THEN
    CREATE ROLE tianxing_test_provisioner NOLOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE
      INHERIT NOREPLICATION NOBYPASSRLS;
  END IF;
END;
$$;

ALTER ROLE tianxing_test_application WITH NOLOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE
  INHERIT NOREPLICATION NOBYPASSRLS;
ALTER ROLE tianxing_test_identity WITH NOLOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE
  INHERIT NOREPLICATION NOBYPASSRLS;
ALTER ROLE tianxing_test_provisioner WITH NOLOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE
  INHERIT NOREPLICATION NOBYPASSRLS;
GRANT tianxing_app TO tianxing_test_application;
...
REVOKE ALL ON TABLE identity_database_test_credentials FROM tianxing_app;
REVOKE ALL ON TABLE identity_database_test_credentials FROM tianxing_test_application;
REVOKE ALL ON TABLE identity_database_test_credentials FROM tianxing_test_identity;
REVOKE ALL ON TABLE identity_database_test_credentials FROM tianxing_test_provisioner;
REVOKE ALL ON TABLE identity_users, identity_sessions, access_organizations,
  access_organization_memberships, access_role_bindings FROM tianxing_test_identity;
REVOKE ALL ON TABLE identity_users, identity_sessions, access_organizations,
  access_organization_memberships, access_role_bindings FROM tianxing_test_provisioner;
...
GRANT USAGE ON SCHEMA public TO tianxing_test_application, tianxing_test_identity,
  tianxing_test_provisioner;
GRANT EXECUTE ON FUNCTION identity_database_test_lookup_credential(text) TO tianxing_test_identity;
GRANT EXECUTE ON FUNCTION identity_database_test_complete_login(uuid, bigint, boolean, uuid, bytea, timestamptz) TO tianxing_test_identity;
GRANT EXECUTE ON FUNCTION identity_database_test_resolve_session(bytea, timestamptz, boolean) TO tianxing_test_identity;
GRANT EXECUTE ON FUNCTION identity_database_test_revoke_session(bytea, text) TO tianxing_test_identity;
GRANT EXECUTE ON FUNCTION identity_database_test_lookup_provision_credential(text) TO tianxing_test_provisioner;
GRANT EXECUTE ON FUNCTION identity_database_test_provision_credential(text, text, bytea, bytea, boolean) TO tianxing_test_provisioner;
