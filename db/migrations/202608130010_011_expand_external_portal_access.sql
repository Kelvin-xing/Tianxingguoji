...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'portal_auth') THEN
    CREATE ROLE portal_auth LOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE NOINHERIT NOREPLICATION NOBYPASSRLS;
  END IF;
END;
$$;

GRANT rds_iam TO portal_auth;
DO $$
DECLARE
  target_database text := current_database();
BEGIN
  EXECUTE format('GRANT CONNECT ON DATABASE %I TO portal_auth', target_database);
END;
$$;

REVOKE ALL ON SCHEMA public FROM portal_auth;
GRANT USAGE ON SCHEMA public TO portal_auth;
REVOKE ALL ON ALL TABLES IN SCHEMA public FROM portal_auth;
REVOKE EXECUTE ON ALL FUNCTIONS IN SCHEMA public FROM PUBLIC;
REVOKE EXECUTE ON ALL FUNCTIONS IN SCHEMA public FROM portal_auth;
ALTER DEFAULT PRIVILEGES IN SCHEMA public REVOKE ALL ON TABLES FROM portal_auth;
