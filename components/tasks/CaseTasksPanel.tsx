      ...
      {state === "ready" && canCreate ? (
        <div className="border-y py-5 space-y-4" style={{ borderColor: "var(--border)" }}>
          ...
          <form className="grid grid-cols-1 md:grid-cols-2 gap-4" onSubmit={submit} aria-busy={pending}>
            <label className="field-label">任務標題<input ref={titleInput} value={title} maxLength={300} required disabled={pending} onChange={(event) => { draftChanged(); setTitle(event.target.value); }} /></label>
            <label className="field-label">負責人<select aria-label="負責人" value={assigneeId} required disabled={pending || optionsState !== "ready"} onChange={(event) => { draftChanged(); setAssigneeId(event.target.value); }}><option value="">{optionsState === "loading" ? "正在載入負責人" : "選擇負責人"}</option>{assignees.map((assignee) => <option value={assignee.id} key={assignee.id}>{assignee.label} · {assignee.role === "advisor" ? "顧問" : "外部協作人員"}</option>)}</select></label>
            <label className="field-label md:col-span-2">工作內容<textarea value={brief} maxLength={4_000} rows={4} required disabled={pending} onChange={(event) => { draftChanged(); setBrief(event.target.value); }} /></label>
            <label className="field-label">到期時間（香港時間）<input type="datetime-local" value={dueLocal} required disabled={pending} onChange={(event) => { draftChanged(); setDueLocal(event.target.value); }} /></label>
