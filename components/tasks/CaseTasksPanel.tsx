...
import { taskAssigneeRoleLabel, TaskListItem, TaskPageState } from "./task-ui";
      ...
      {state === "ready" && canCreate && createOpen ? (
        <div id="case-task-create-form" className="border-y py-5 space-y-4" style={{ borderColor: "var(--border)" }}>
          ...
          <form className="grid grid-cols-1 md:grid-cols-2 gap-4" onSubmit={submit} aria-busy={pending}>
            ...
            <label className="field-label"><span>負責人 *</span><select aria-label="負責人" value={assigneeId} required disabled={pending || optionsState !== "ready"} onChange={(event) => { draftChanged(); setAssigneeId(event.target.value); }}><option value="">{optionsState === "loading" ? "正在載入負責人" : "選擇負責人"}</option>{assignees.map((assignee) => <option value={assignee.id} key={assignee.id}>{assignee.label} · {taskAssigneeRoleLabel(assignee.role)}</option>)}</select></label>
