...
export function CaseIntakeWorkspace() {
  ...
  if (state === 'error') return <ErrorState title="Case 建立未完成" detail={errorCode === 'VALIDATION_FAILED' ? '请完成 Student、Advisor、intake year 与 signed_at。' : errorCode === 'CONFLICT' ? '该 Student 已有相同学年和申请类型的活动 Case；请调整 Intake year 或 Admission type。' : `服务端返回 ${errorCode ?? 'CREATE_FAILED'}；请修正后重试，不会自动重放旧命令。`} onRetry={() => setState('ready')} />
