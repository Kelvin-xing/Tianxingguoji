...
test("registers exactly the twelve approved capability-backed navigation routes", () => {
  assert.deepEqual(
  ...
  assert.equal(new Set(NAVIGATION_REGISTRY.map(({ route }) => route)).size, 12);
