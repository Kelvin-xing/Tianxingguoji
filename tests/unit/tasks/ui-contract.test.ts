...
test("Task detail owns transition success after authoritative refresh without duplicate child status", async () => {
  ...
  assert.match(transitions, /classifyTaskFailure\(error\) === "not_found"/);
  assert.match(transitions, /onAssignmentEnded\(\)/);
  ...
  assert.match(detail, /state === "assignment_ended"/);
  assert.match(detail, /此指派已結束，任務已從你的工作清單移除。/);
  assert.match(detail, /返回任務清單/);
  ...
  assert.match(automatic, /classifyTaskFailure\(error\) !== "not_found"/);
  assert.match(automatic, /onAssignmentEnded\(\)/);
