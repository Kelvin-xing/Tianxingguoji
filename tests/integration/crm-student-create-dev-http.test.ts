...
test("CRM-01 works through PostgreSQL 17 and the real local Next Dev HTTP API", {
  timeout: 300_000,
}, async () => {
  ...
  try {
    ...
    assert.equal(access.body.data?.policy_version, "release1-bootstrap-v2");
    ...
    assert.equal((detail.body.data?.student as { guardians?: Array<{ id?: string }> })
      .guardians?.some(({ id }) => id === guardianId), true);
...
function assertApiError(
  result: Awaited<ReturnType<typeof createStudent>>,
  status: number,
  code: string,
): void {
