...
## 当前推进入口（2026-09-19，员工邀请页面与重复操作接通阶段）

本文件后续章节按阶段保留历史快照；旧章节的“未完成/下一步”仅代表当时状态，不能覆盖较新证据。完整目标仍是原计划 A–E 开发与本地验收、合并 main、推送远程；F 部署保持独立授权。

1. **员工邀请已完成本阶段链路。** 新等级表单、L2 分类、当前目录权限、旧角色编辑保护、创建/重发操作记录及未知投递结果处理均已接通，见末尾最新本地证据。后续整体验收仍需与完整计划逐条核对。
2. **剩余范围审计与补齐：** 目标结果/结案、通知、业务/安全审计、门户入口，逐项核对 BR-015 与原计划，只补必要权限缺口，不扩展未确认业务。
3. **整体验收与演示：** 五个合成身份、两分类及跨分类任务、未授权数据不可见、撤权即时生效；定位偶发上传路由404。此错误多次复测未复现，仍不能声称根因已修复。
4. **最终交付：** 逐条核验计划、正式需求的本任务改动、安全与本地证据；保留原 main 工作区无关修改地合并本分支，推送并核实远程 SHA。当前尚未执行合并/推送。
...
## 学校审批页面与决定履历（2026-09-19，接续 4564d19）

总体仍 `in_progress`。上一轮属于实质进展：提交审批事务并完成正式HTTP验证；本轮将该行为接通到实际页面。

- 学校待处理记录显示提交人、提交时有效值、当前有效值、申请值及证据。服务端按当前实际权限和申请者分离返回允许动作，只有有权的其他审批人可操作；申请版本/字段基线已过期或旧记录缺少基线时不返回批准动作，保留拒绝入口并解释原因。历史学校不在当前快照中时仍可读申请履历，直接批准返回版本冲突。
- Founder/L1页面可批准或拒绝，须填写理由并核对资料。正在填写或结果未知时锁定其他操作，响应丢失后以原决定和原幂等键重试；权限失效清空资料；冲突须重新载入并重新确认。修复申请/审批表单在“结果未知→重试收到冲突”时仍保留pending、无法关闭的问题，不复用原确认。
- 决定履历展示实际审批人/等级、决定、理由及时间，保留原申请。旧审批无新receipt或无姓名的记录不伪造补全。客户端严格核对审批回执的学校、申请、revision、决定、版本与resolved ID，拒绝多余字段；读取契约校验动作、决定、时间和旧值证据。
- 追加073保存提交时有效字段的JSON值，区分已知的未知值（jsonb null）与旧记录未保存（SQL NULL）；不回填虚构历史。沿用字段不可变保护，服务端在同一事务内先核对哈希再保存真实当前值。72源迁移、73生成文件，基线检查通过。
- `/tmp/access-trial-school-review-ui-browser3.log` **51/51**：真实PG17、Next/Chrome、客户端、迁移、模块边界。浏览器实际拒绝、批准、理由/确认校验、提交成功后模拟503再同键重试、刷新后决定和旧值保留、L2无审批按钮、真实并发更新后先丢失409响应再重试409、关闭重新核对且确认框清空、模拟403清空页面、390px无横向溢出均通过。既有CRM、任务、上传扫描、文件及邀请回归通过。
- 同组真实PG新增验证权限动作、自审无按钮、拒绝记录含实际姓名/等级/理由、缺少基线只准拒绝、保存未知/已有有效值的区别、无当前快照的历史记录读取及批准拒绝。早期新增SQL夹具的参数类型歧义已修正；后续真实PG通过。审批表单textarea改用明确关联的标签，修正填入内容后的精确名称定位问题。
- 类型 `/tmp/access-trial-school-review-ui-types3.log`、聚焦ESLint `/tmp/access-trial-school-review-ui-lint.log`及最终变动ESLint `/tmp/access-trial-school-review-ui-lint-final.log`、diff通过。已查看最新 `/tmp/access-trial-school-review-ui-mobile.png`。最后仅把来源说明改为业务用语，未再重复浏览器测试。
- 中间浏览器组学校审批页面通过，随后匿名上传意图路由预检查返回404（预期401）。没有放宽状态码或重试业务上传；增加失败时响应类型/安全错误码、源文件/编译产物/路由清单存在性诊断。最终复测通过，但偶发404根因仍未证明修复，必须继续排查，不将预检查当作修复。

后续继续核对计划中的剩余权限入口、合成演示与整体本地验收，并处理已知运行稳定性问题，再合并main并推送。当前未合并/推送，未部署或操作真实员工及客户资料。

## 上传路由匿名请求隔离与开发诊断（2026-09-19，接续 c146199）

总体仍 `in_progress`，未满足合并条件。

- 新增独立真实Next Dev HTTP用例，复用现有隔离目录/启动/清理助手，不需要数据库。冷启动并发请求任务上传意图、案件上传意图、任务放弃版本和新建版本；再使用未认证的无效参数，均必须返回JSON 401/UNAUTHENTICATED。没有重试业务上传、扩大许可或放宽断言。
- 首次探测案件路径返回422，原因是空参数先被校验，并非404复现；使用合法上传意图参数后稳定返回503。源码确认在await身份前先求值getDocumentTransferRuntime()，依赖不可用会覆盖未登录响应。案件上传意图路由现先取得当前actor，再解析请求和初始化文件服务，合法与无效未认证请求均401。没有声称此问题解释此前任务路径偶发HTML 404。
- 依据已安装Next16.2.7配置实现修正诊断读取路径为`.next/dev/server`。旧`.next/server`会错误报告清单与产物缺失；本次失败诊断确认新路径能找到任务路由清单与编译产物。未改Next配置或业务运行模式。
- `/tmp/access-trial-document-auth-final.log` **1/1**：八个独立HTTP请求的冷启动/并发及无效参数未登录拒绝。`/tmp/access-trial-document-auth-browser.log` **2/2**：真实一次性PG17、Next/Chrome，既有员工等级、CRM、学校审批、任务、文件传输与扫描、撤权等链路通过。偶发404本次未复现，根因仍未证明修复。
- 类型`/tmp/access-trial-document-auth-types.log`、聚焦ESLint`/tmp/access-trial-document-auth-lint.log`与最终测试格式检查`/tmp/access-trial-document-auth-lint-final.log`、diff通过。最终仅整理新测试格式和显式诊断类型，未重复整组浏览器。无新增迁移。
- 剩余入口的只读复核确认：员工邀请仍使用旧role契约及`identity_internal_email_create_invite`，尚不支持直接指定试用等级/分类；Email settings应用层和写入重验仍要求Admin，新Founder尚不可配置。下一步必须按BR-015接通，不能把此前面试邀请测试当成员邀请验收。通知、审计、门户、目标/结案及演示数据仍需有界审计后完成原计划对应验收，不扩展未确认业务。

未合并main、未推送、未部署、未发送外部邮件、未操作真实员工或客户资料；原工作区无关修改保持不动。

## 邮件配置与邀请模板的试用权限（2026-09-19，接续 0ffcacb）

总体仍 `in_progress`。上一轮有实质进展：上传意图先验证登录，并完成真实本地回归。本轮接通原计划中的系统配置权限，尚未接通新等级员工邀请。

- Email settings/template应用服务对显式试用成员按当前能力授权，不再额外要求旧Admin角色。真实仓储读写、重放和内部发送配置读取均重新锁定/检查当前试用状态、用户、组织、membership及实际role binding；试用Founder允许，L1/L2/L3拒绝，禁用/撤权不能利用旧请求上下文或旧回执继续操作。
- 管理读取与邀请发送读取明确分开：新Founder两者可用；旧Admin仍可管理，但不能作为Founder发送邀请；旧Founder可在内部发送路径读取配置/模板，不自动得到管理权限。新增模板`readDeliveryTemplate`入口并接入原发送resolver，避免管理权限检查误伤旧Founder邀请。无外部邮件发送。
- 重放回执从幂等记录关联的不可变审计重建原版本/时间，校验原response hash与result reference，不再使用重试时间冒充原更新时间。后续版本已改变时，旧请求仍返回原回执；当前授权检查在回执读取前完成。
- 两个页面保存期间冻结输入，结果未知时保留原命令及幂等键，只能重试原提交；成功后清除密钥输入，读取不回显明文/密文。版本冲突进入重新载入，丢弃旧命令；保存时401/403清空配置编辑区、密钥或模板预览。权限提示不再错误宣称只有Admin可用，字段有明确可访问名称。
- `/tmp/access-trial-email-pg-final2.log` **2/2**：一次性PG17真实基线。覆盖四等级管理/发送读取、伪造Founder上下文、无权直接仓储读取、保存/重放/版本冲突、审计失败原子回滚后同键重试、后续修改后的原回执、禁用后读写及重放拒绝、旧Admin实际写入和旧Founder发送兼容；密钥加密且状态/审计不回显。初期夹具停用遗漏session_version递增已修正；单个外层回滚事务的初始创建时钟改为该事务数据库时间，避免测试夹具与既有UPDATE触发器时钟不一致，未因此修改生产触发器或历史迁移。
- `/tmp/access-trial-email-browser-final.log` **30/30**：真实PG17+Next/Chrome、Email单元/契约与模块边界。实际Founder保存并刷新两页面；提交已成功但响应模拟503后原键重试只产生一版；L1/L2正式GET/PUT403，L1工作区入口拒绝；真实并发新版本使旧表单409，重新载入恢复且旧密钥清空；模拟保存403后编辑区/预览清空；390px无溢出。既有CRM/学校审批/任务/上传扫描/撤权等完整链路继续通过，上传404未复现但仍未定位。
- `/tmp/access-trial-email-legacy-browser.log` **1/1**：旧Founder邀请/链接轮换/激活/登录/页面邀请重发及旧Admin邮件配置、模板管理真实PG与浏览器回归通过。全部合成账号、确定性假邮件传输，无外部投递。
- 早期浏览器失败是测试期待页面内拒绝文案，而实际由AppFrame统一工作区权限拦截；修正为验证实际拒绝入口，并保留直接API403和无编辑区断言。后续两轮浏览器通过；最终一轮新增真实版本冲突恢复检查。
- 类型`/tmp/access-trial-email-types-final3.log`、聚焦ESLint`/tmp/access-trial-email-lint.log`、`/tmp/access-trial-email-lint-final.log`、`/tmp/access-trial-email-lint-final2.log`与diff通过。已目视检查`/tmp/access-trial-email-settings-mobile.png`、`/tmp/access-trial-email-template-mobile.png`的手机页面。无新增迁移，仍为72源迁移/73生成文件。

下一项：`modules/identity/application/internal-email.ts`、`postgresql-internal-email-repository.ts`、`app/api/v1/auth/invites/route-contract.ts`及`/admin/access`邀请表单目前仍用旧角色；需追加数据库适配并贯通新等级/分类、邀请与激活。现有`identity_internal_email_create_invite`在055迁移，不能改历史文件。剩余范围审计/演示验收与main合并推送继续按本文件顶部清单执行；未部署或处理真实员工数据。


## 员工邀请后端阶段（2026-09-19，接续 6bc0377）

- status: `partial`；owner: backend。范围为 BR-014/015/075 员工邀请服务、正式 HTTP 接口及追加迁移 074。客户部署、真实邮箱和真实员工迁移均未执行。
- 试用 Founder 必须明确指定 Founder/L1/L2/L3 及分类数组，只有 L2 可非空；兼任/正式员工不限制试用等级。旧 Founder 保留原四角色请求，显式试用 Founder 不能发旧角色邀请。SQL 事务内重新锁定当前邀请者、组织、成员和角色，拒绝伪造/过期 Founder 事实；重发同样重新鉴权。
- 创建顺序保持员工资料先于有效角色绑定；试用记录与用户、成员、角色、邀请在同一事务。激活前用户/成员仍为 invited，不能登录。激活验证当前试用状态和实际角色；一次性链接重发失效，使用后不能重用。L3 激活成功直接进入“我的任务”。
- 原邀请身份和初始分类不可修改；创建、重发、激活的审计和 outbox 与数据库变更同事务，禁止明文凭据进入事实。审计失败时分别验证创建无残留、激活的用户/成员/凭据/会话全部回滚，恢复审计后原链接可成功激活。
- 修复两个真实 HTTP 错误映射问题：内部邮箱服务改为模块内缓存，避免跨 Next 路由模块持有不同错误构造器而将 FOUNDER_REQUIRED 错报 503；邀请路由保留访问上下文 401/403，不再在外层统一改成 503。数据库连接池仍沿用原全局复用。
- 本地证据：`/tmp/access-trial-invite-backend-final3.log` **27/27**，包括真实 PostgreSQL 17 + Next HTTP + 原员工/邮件浏览器链路、四等级创建/重发/激活/重新登录、L1/L2/L3 创建及重发 403、旧会话 401、当前 Founder 旧角色请求 422、伪造 Founder 仓储拒绝、停用待激活成员拒绝、审计回滚；并含邀请/激活契约、单元及模块边界测试。
- 基线证据：`/tmp/access-trial-invite-baseline-final.log` **30/30**，空库 dry-run 回滚、真实安装和独立连接验证；73 个源迁移、74 个生成文件、625 个 public 对象，错误 owner / 未强制 RLS / 不安全 definer 均为 0。历史迁移未修改。
- 类型检查 `/tmp/access-trial-invite-types-final.log` 通过，聚焦 ESLint `/tmp/access-trial-invite-lint-final.log` 通过，`git diff --check` 通过。
- **未完成**：邀请 UI 仍使用旧角色表单；需要服务端提供当前邀请模式，接等级/L2 分类，防止旧 MemberEditor 改试用等级。目录仓储需要当前权限复核。创建 idempotencyKey 仍是现有未落库实现，重发 key 也只校验未持久化；必须补齐重复请求/未知投递结果处理，不能声称整个邀请功能完成。没有新增明文或加密令牌持久化方案。
- 目标保持 active，未合并 main、未推送、未部署。后续继续按顶部入口推进。


## 员工邀请页面与重复操作阶段（2026-09-19，接续 9f22c63）

- status: `pass`（本阶段本地证据）；整体目标仍 `in_progress`。owner: backend/frontend。范围为 BR-014/015/075 已确认员工邀请及既有重复副作用约束，不新增邮件供应商、秘密持久化方案或后台发信任务。
- `/admin/access` 按服务端返回的邀请模式显示四个试用等级或旧角色。L2 可选择两个业务分类，FULL_TIME/PART_TIME 不限制试用等级；服务端目录明确返回每位员工是否已进入试用等级，含 Founder，旧编辑器不再误判 Founder 为可改旧角色。旧角色写仓储在组织锁内检查当前邀请者/目标状态，禁止通过旧命令更改任何试用成员。
- 用户目录在真实事务内重新检查当前试用身份及实际 active 用户/组织/成员/角色；显式试用只允许 Founder，旧 Founder/Admin 保留目录兼容。直接仓储调用的 L1 也被拒绝。目录服务缓存改为模块内，403/401 不再被外层错误处理改写成 503。拒绝加载时清空旧员工数据及邀请入口。
- 追加迁移 075：`identity_invite_operations` 保存每个组织/操作者/操作/请求键的请求摘要、不可变邀请版本和投递回执。只有哈希，无原始激活令牌、密码或加密令牌保存。创建/轮换与操作记录在同一事务；相同键重试先重新鉴权，再返回原记录，不重复创建、轮换或发送邮件；不同载荷同键返回 409。
- 邮件外部副作用只由成功创建该操作的调用执行一次。投递失败或进程中断后，原操作重试返回 `delivery_status=unconfirmed` 和空回执；操作者明确重发才产生新链接并使旧链接失效。已确认回执不可覆盖；旧操作回执在后续重发后仍保持原值。写回执时锁定邀请，较晚返回的旧回执不会覆盖最新邀请的投递摘要。
- 页面在响应不明时冻结原输入并复用原请求键，通过“确认邀请结果/确认重发结果”查询原操作；不会把每次网络重试当作新发信操作。未确认投递时给出待邀请记录和明确重发提示。移动端邀请框可滚动；修复员工表格绝对定位的 sr-only 标签逃出滚动容器、导致页面被撑到 767px 的问题。
- `/tmp/access-trial-invite-operations-final.log` **32/32**：真实 PostgreSQL 17 + Next HTTP + Chrome；四等级创建/激活/登录，重发旧链接失效，Founder/非 Founder 拒绝，目录直接仓储拒绝，旧 Admin 更改试用 Founder 拒绝，审计失败回滚；独立 Node/Next 连接池同时使用同一创建键只有一个操作，同键不同载荷 HTTP 409；并发重发只增一个邀请版本；未知投递原操作不再发送、明确重发恢复；迟到回执不覆盖新回执。
- 同批浏览器覆盖 L2 两分类、兼职、提交/刷新持久化，邀请和重发分别模拟服务端已提交但返回 503，再以相同请求键/正文确认，数据库仅一条创建及一次版本递增；403 重载清空数据/入口。`/tmp/access-trial-invite-mobile.png` 已目视检查，390px，无横向溢出。旧员工激活/登录/昵称/角色和邮件配置浏览器流程同时通过。
- `/tmp/access-trial-invite-operations-baseline-final.log` **30/30**：74 源迁移/75 生成文件、631 public 对象；dry-run 回滚、真实安装及独立连接验证，错误 owner / 未强制 RLS / 不安全 definer 均为 0。075 复用已有仅安装期间开放 REFERENCES 的模式；历史迁移未修改。
- `/tmp/access-trial-invite-full-browser.log` **2/2**（约142秒）：完整现有权限 PG+浏览器回归，包含等级、建案、CRM、学校审批、邮件、L3 任务、文件上传与撤权。此次未复现偶发上传404，仍不声称其根因已修复。
...
## 审计页面与门户/目标边界复核（2026-09-19，接续当前阶段）

总体仍 `in_progress`。本节补齐内部审计读取页面，并记录两个明确的运行边界。

- 新增 `/audit` 内部页面和 `audit.read` 工作台能力。Founder/L1/L2/L3 以及历史 Founder/Admin/Advisor 可进入页面；业务与安全范围由服务端重新读取当前组织、成员、试用等级和实际角色后决定，页面不把导航可见当作授权。L2 当前仍遵循审计事件尚无权威业务分类字段的保守规则，只显示本人业务操作，待字段进入事件事实后再扩展分类范围；L3 只显示本人 `tasks.*` 事件；安全审计只给 Founder。
- 页面支持业务/安全范围切换、时间游标加载更早记录、空/拒绝/服务不可用/失败状态和移动端横向滚动；只展示事件类型、动作、资源不透明标识、结果、请求号、时间、操作人标识和脱敏标量 metadata，不展示自由文本 PII、文件内容、Cookie、token 或 secret。
- 审计客户端、页面、导航契约、访问能力矩阵和 `release1-bootstrap-v17` 均通过类型检查、聚焦 ESLint 和单元测试。真实 PostgreSQL 读取证据仍为本文件上一节记录的 Founder/L1/L2/L3 业务范围、Founder-only 安全范围和游标不重叠。
- 门户复核未改代码：BR-060/063 的单 Case、只读白名单、当前 Primary Advisor 创建/撤销、Founder 紧急撤销、7 天入口和到期/撤销即时失效保持成立；门户相关单元、契约、仓储、路由和 schema **38/38** 通过。试用 L1/L2/L3 未被擅自映射为 Primary Advisor，避免引入未经客户确认的新门户规则。
- 目标结果/结案继续遵守已批准的 P2-03 fail-closed 方案：正式路由和 `CaseOutcomeRuntime` 在没有批准的香港 RDS 事务适配器时返回冲突/不可用；聚焦契约与 outcome service **8/8** 通过。没有实现本地内存、JSON、旧数据库或云端回退。
...
本轮开发、真实 PostgreSQL/Next/Chrome 整体验收、合并到 `main` 和推送已完成；当前 `HEAD` 与 `origin/main` 均为 `e78e1453c7725834d343d6df2971f7ddd19839a7`。目标结果正式事务运行时仍受已批准的香港生产数据库适配边界限制；客户测试环境部署、真实员工/客户资料迁移和外部邮件发送不在本轮授权内。
