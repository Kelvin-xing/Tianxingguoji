...
Contractors need enough information to accept, deliver, or return any currently assigned Task, including an explicitly created manual Task, without receiving the surrounding Service Case. The contractor is the direct stakeholder; the Founder, Primary Advisor, family, privacy owner, and security owner need assignment revocation and data minimization to hold at the server boundary rather than only in navigation or React rendering.
