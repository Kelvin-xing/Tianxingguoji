...
草案修订：2026-09-21（取消数值 confidence 必填；协议标识仍为 2.0，尚未冻结）。

状态：`proposed`，待项目负责人在 Stage A 技术审阅。本文产出技术契约草案及可执行的双侧校验，不表示实现了一份已批准设计，也不表示契约已获批准或冻结。

唯一业务依据：txgj-doc 的 **BR-051 / `BR-BASELINE-20260920-v56`**（学校字段 V1、D4、N2）。

## BR-051 原文与审阅范围

原文：[BR-051 学校目录与数据变更](/Users/karo/Documents/txgj-doc/business-requirements/50-schools.zh-CN.md)。以下直接摘录 v56，技术字段名和校验规则是本文提出的表达方式，不将它们反向提升为已批准业务规则。
...
参考资料与既往分析：SCH-INT-01（包括 §8.3、§11）、SCH-INT-02/03/05 仅供阅读、借鉴和讨论，不构成业务依据、技术批准或实施授权。SCH-INT-04 的结论用于说明审阅安排：接口格式不是业务门禁，而是 A 阶段需要技术审阅的产出。本草案的版本、字段编码、状态、年级集合、原始材料表达及错误协议均待本次审阅；双侧校验通过不能替代负责人批准。
...
- `contract.schema.json` 定义全部结构、字段与类型；下述语义规则同为本草案的校验约定。两侧离线校验器先验结构，结构通过再验语义；普通 JSON Schema 工具只验结构不足以验收。
- 属性一律显式提供，不认识的属性拒绝；禁止省略字段来暗示“沿用”或“删除”。审阅期间可修订本草案并同步两侧样本与校验器；正式批准后的兼容性策略建议为：增删字段、改变可接受集合或解释必须新增契约版本，旧版本不静默变义。
...
`text_quote` 是原材料摘录，`locator` 是段落/页码/JSON 路径等定位；confidence 是可省略的 0–1 数值评分；如提供则不接受布尔值、类别或超范围值，不具有业务授权效力。缺失数值评分时省略，不填默认值，不把 high/medium/low 换算成数字；原有类别评分逐字保留在 raw_records.content 中。BR 原文：“注册详情、课室容量、PDF 原文、抓取评分等保留在原始记录中，不作为首版主要业务字段。”因此评分不作为字段证据的必填门槛。包内可表达未知、失败和旧证据，均不能用来清空系统旧值。结构校验不证明摘录真实或足以支持所填事实。
