...
import { listCases } from '@/components/crm/f2-contract'
...
export default function CasesPage() {
  ...
  function loadCases() {
    ...
    listCases()
      .then((items) => {
        setCaseRecords(items.map((item) => ({
          id: item.id,
          caseNumber: item.case_number,
          studentId: item.student_id,
          studentName: item.student_name,
          intakeYear: item.intake_year,
          admissionType: item.admission_type,
          stage: item.stage as CaseStage,
          updatedAt: item.updated_at,
          primaryRole: 'advisor',
        })))
        setLoadState('ready')
      })
      .catch((error: unknown) => setLoadState(error instanceof ApiClientError && error.code === 'SERVICE_UNAVAILABLE' ? 'error' : 'error'))
  }
  ...
  return (
    <div className="max-w-[1500px] mx-auto space-y-6">
      ...
      <section className="workspace-section overflow-hidden">
        ...
        {loadState === 'error' && <div className="empty-state"><Icon name="x" size={20} /><strong>案件服務暫時不可用</strong><span>CRM/Cases transport 尚未可用；不会使用 preview adapter。</span><button type="button" className="secondary-button mt-3" onClick={loadCases}>重新載入</button></div>}
