...
export default function EmailSettingsPage() {
  ...
  const [attempt, setAttempt] = useState<{ key: string; body: { api_key: string; from_email: string; from_name: string | null; expected_record_version: number | null } } | null>(null)
  ...
  async function save() {
    if (!attempt && (!settings || !/^re_[A-Za-z0-9_-]{8,252}$/.test(apiKey.trim()) || !/^\S+@\S+\.\S+$/.test(fromEmail.trim()))) {
      setMessage('請輸入有效的 Resend API 密鑰和發件電郵。')
      return
    }
    const submitted = attempt ?? {
      key: `email-settings-${crypto.randomUUID()}`,
      body: { api_key: apiKey.trim(), from_email: fromEmail.trim().toLowerCase(),
        from_name: fromName.trim() || null, expected_record_version: settings!.recordVersion },
    }
    setAttempt(submitted)
    setSaving(true)
    setMessage(null)
    try {
      await requestApi({ path: '/api/v1/email/settings', method: 'PUT',
      ...
      if (error instanceof ApiClientError && (error.code === 'STALE_VERSION' || error.code === 'CONFLICT')) { setAttempt(null); setApiKey(''); setState('error'); setMessage('設定已變更，請重新載入後再核對。') }
      else if (error instanceof ApiClientError && (error.code === 'FORBIDDEN' || error.code === 'UNAUTHENTICATED')) { setAttempt(null); setSettings(null); setApiKey(''); setFromEmail(''); setFromName(''); setState('denied') }
      ...
      else setMessage('儲存結果尚未確認，請重試原設定。')
    } finally {
      setSaving(false)
    }
  }
  ...
  return <div className="max-w-3xl mx-auto space-y-6">
    ...
    {state === 'denied' && <ErrorState title="無法查看郵件設定" detail="你目前沒有管理郵件設定的權限。" />}
    {state === 'unavailable' && <UnavailableState title="郵件設定服務暫時不可用" detail="請聯絡系統管理員確認郵件服務設定。" onRetry={load} />}
    {state === 'error' && <ErrorState title="郵件設定讀取失敗" detail={message ?? "請稍後重試。"} onRetry={load} />}
    ...
    {state === 'ready' && settings ? <section className="workspace-section space-y-5">
      ...
      <label className="block"><span className="text-sm font-medium">Resend API 密鑰</span><input aria-label="Resend API 密鑰" disabled={saving || attempt !== null} className="mt-2 w-full" type="password" value={apiKey} autoComplete="new-password" placeholder={settings.configured ? '輸入新密鑰以輪換目前設定' : 're_…'} onChange={(event) => { setApiKey(event.target.value); setMessage(null) }} /><span className="form-help">為安全起見，已儲存的密鑰不會顯示。</span></label>
      <label className="block"><span className="text-sm font-medium">發件電郵</span><input aria-label="發件電郵" disabled={saving || attempt !== null} className="mt-2 w-full" type="email" value={fromEmail} placeholder="accounts@example.com" onChange={(event) => { setFromEmail(event.target.value); setMessage(null) }} /><span className="form-help">必須屬於已在 Resend 驗證的域名。</span></label>
      <label className="block"><span className="text-sm font-medium">發件名稱</span><input aria-label="發件名稱" disabled={saving || attempt !== null} className="mt-2 w-full" value={fromName} maxLength={100} onChange={(event) => { setFromName(event.target.value); setMessage(null) }} /></label>
      ...
      <div className="flex justify-end"><button type="button" className="primary-button" disabled={saving || (!attempt && (!apiKey.trim() || !fromEmail.trim()))} onClick={() => void save()}><Icon name="check" size={16} />{saving ? '儲存中' : attempt ? '重試原設定' : settings.configured ? '輪換並儲存' : '儲存設定'}</button></div>
    </section> : null}
  </div>
}
