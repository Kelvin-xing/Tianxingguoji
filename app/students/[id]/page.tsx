  ...
  return (
    <div className="space-y-5 max-w-4xl">
      <div className="flex items-center gap-1.5 text-xs" style={{ color: 'var(--text-muted)' }}>
        <Link href="/students" style={{ color: 'var(--accent)' }}>
          {t('nav.students')}
        </Link>
        <span>/</span>
      ...
      >
        ...
        <span
          ...
          style={STATUS_STYLES[student.status]}
        >
      ...
      <div
        ...
        style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}
      >
        <h3 className="text-xs font-semibold mb-4" style={{ color: 'var(--text-secondary)' }}>
          申請進度
        </h3>
              ...
              <div key={step} className="flex-1 flex items-center">
                <div className="flex flex-col items-center flex-1">
                  <div
                    ...
                    style={{
                      background: active ? 'var(--accent)' : past ? '#10b981' : '#e5e7eb',
                      color: active || past ? '#fff' : 'var(--text-muted)',
                    }}
                  >
                    {past ? '✓' : i + 1}
                  </div>
                  <div className="text-xs mt-1.5" style={{ color: 'var(--text-secondary)' }}>
                    {t(`students.status_${step}`)}
                  </div>
                </div>
                {i < STATUS_FLOW.length - 1 && (
                  <div
                    className="h-0.5 flex-1 -mt-5"
                    style={{ background: i < currentStatusIdx ? '#10b981' : '#e5e7eb' }}
                  />
                )}
              </div>
            )
            ...
            {student.documents.map((doc) => (
              ...
              >
                ...
                <span
                  ...
                  style={{ color: doc.uploaded ? '#15803d' : '#dc2626' }}
                >
          ...
          {targetSchoolData.map((school) => {
            ...
            return (
              <div
                ...
                className="flex items-center justify-between text-sm p-3 rounded-md"
