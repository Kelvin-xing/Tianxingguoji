...
import { RequestInvalid, authenticateInternalPortalRequest, isIsoDate, mapInternalError, portalJson, readIdempotencyKey, readObject, requireTrustedMutationOrigin } from "../../route.ts";
...
export function createPortalGrantRotateHandler(deps: PortalGrantRouteDependencies) {
  return async (request: Request, context: ItemContext) => {
    try {
      ...
      if (!Number.isSafeInteger(body.expected_version) || Number(body.expected_version) < 1 || !isIsoDate(body.expires_at)) throw new RequestInvalid();
      ...
      const result = await deps.rotateGrant({ actorUserId: actor.actorUserId, caseId, grantId, expectedVersion: Number(body.expected_version), expiresAt: body.expires_at, idempotencyKey });
