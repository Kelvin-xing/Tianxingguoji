...
export function mapInternalError(error: unknown): Response {
  ...
  if (error instanceof UntrustedMutationOrigin) {
    return portalJson({ error: { code: "PORTAL_ACCESS_DENIED" } }, 403);
  }
  if (error instanceof IdentityRuntimeUnavailable || error instanceof PortalRuntimeUnavailable) {
  ...
  if (error instanceof PortalPolicyError) {
    if (error.code === "PORTAL_VERSION_CONFLICT") {
      return portalJson({ error: { code: "PORTAL_VERSION_CONFLICT" } }, 409);
    }
    if (error.code === "PORTAL_INPUT_INVALID" || error.code === "PORTAL_EXPIRY_INVALID") {
      return portalJson({ error: { code: "PORTAL_REQUEST_INVALID" } }, 400);
    }
    return portalJson({ error: { code: "PORTAL_ACCESS_DENIED" } }, 403);
  }
  if (error instanceof PortalRepositoryError) {
    ...
    if (
      error.code === "PORTAL_IDEMPOTENCY_KEY_REUSED" ||
      error.code === "PORTAL_GRANT_NOT_ACTIVE"
    ) {
      return portalJson({ error: { code: "PORTAL_CONFLICT" } }, 409);
    }
  }
  return portalJson({ error: { code: "PORTAL_RUNTIME_UNAVAILABLE" } }, 503);
}
...
function toGrantJson(grant: PortalGrantSummary) {
  return {
    ...
    portal_viewer_id: grant.portalViewerId,
    fingerprint: grant.fingerprint,
    expires_at: grant.expiresAt,
    status: grant.status,
    record_version: grant.recordVersion,
  };
}
