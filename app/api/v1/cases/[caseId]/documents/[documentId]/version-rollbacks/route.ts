...
import {
  ...
  type RollbackDocumentVersionCommand,
...
export async function POST(
  ...
  return handleApiRequest(request, async (requestContext) => {
    ...
    try {
      ...
      const result = await getDocumentVersionRuntime().service.rollbackToCleanVersion({
        actor,
        caseId,
        documentId,
        command,
...
async function parseCommand(request: Request, requestId: string): Promise<RollbackDocumentVersionCommand> {
  ...
  if (!isRecord(body) || typeof body.target_version_id !== "string" || typeof body.expected_record_version !== "number") {
