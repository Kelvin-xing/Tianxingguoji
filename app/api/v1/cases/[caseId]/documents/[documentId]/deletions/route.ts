...
export async function POST(
  ...
  return handleApiRequest(request, async (requestContext) => {
    ...
    try {
      ...
      const result = await getDocumentVersionRuntime().service.softDeleteDocument({
        actor,
        caseId,
        documentId,
        command,
...
async function parseCommand(request: Request, requestId: string): Promise<SoftDeleteDocumentCommand> {
  ...
  let body: unknown;
  try {
    body = await parseDocumentLifecycleBody(request,["expected_record_version"]);
  } catch {
    throw createApiError("INVALID_REQUEST");
  }
  if (!isRecord(body) || typeof body.expected_record_version !== "number") {
