...
export interface PortalWorkspaceRouteDependencies {
  ...
  readWorkspace(input: { readonly sessionSecret: string }): Promise<PortalCaseReadV1 & Record<string, unknown>>;
}
  ...
  return async function GET(_request: Request): Promise<Response> {
    try {
      ...
      return json({
        ...
        school_targets: value.schoolTargets.map((item) => ({ name: item.name, status: item.status })),
        action_items: value.actionItems.map((item) => ({ title: item.title, deadline: item.deadline, completed: item.completed })),
        messages: value.messages.map((item) => ({ body: item.body, published_at: item.publishedAt })),
      }, 200);
    } catch (error) {
      if (error instanceof PortalRuntimeUnavailable) return json({ error: { code: error.code } }, 503);
...
const defaultGet = createPortalWorkspaceGetHandler({
  getSessionSecret: async (request) => readCookie(request?.headers.get("cookie") ?? null, PORTAL_SESSION_COOKIE_NAME) ?? null,
