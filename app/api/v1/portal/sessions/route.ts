...
export interface PortalSessionRouteDependencies {
  redeem(input: { readonly accessKey: string }): Promise<{ readonly sessionSecret: string; readonly absoluteExpiresAt: string }>;
  ...
  return {
    POST: async (request: Request): Promise<Response> => {
      try {
        ...
        if (typeof accessKey !== "string" || accessKey.length < 16 || accessKey.length > 1024) return invalidRequest();
    ...
    DELETE: async (request: Request): Promise<Response> => {
      ...
      const clearCookie = { "Set-Cookie": `${PORTAL_SESSION_COOKIE_NAME}=; Max-Age=0; ${COOKIE_OPTIONS}` };
      try {
...
function publicError(error: unknown): Response {
  ...
  if (error instanceof PortalRepositoryError) {
    code = error.code === "PORTAL_SESSION_LIMIT_REACHED" ? "PORTAL_SESSION_LIMIT_REACHED" : "PORTAL_SECRET_INVALID";
  }
